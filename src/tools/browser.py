...
使用 Playwright 实现真实网页浏览，封装为 LangChain @tool 供 Agent 节点调用。

使用方式:
    from src.tools.browser import BROWSER_TOOLS, browse_web_page
"""
...
async def _fetch_page_screenshot(url: str) -> str:
    """异步获取网页截图并返回 base64。"""
    import base64
    from playwright.async_api import async_playwright
...
def screenshot_web_page(url: str) -> str:
    ...
    try:
        ...
        result = _run_async(_fetch_page_screenshot(url))
